...
- Public exported types are reserved for cross-package contracts or adapters, for example `app.Info`, `chezmoi.Client`, `chezmoi.StatusEntry`, `diff.Differ`, `process.Runner`, and `tui.Action`.
...
- Enum-like values use a private integer type plus exported constants when used across packages: `tui.ActionKind` with `ActionAdd`, `ActionApply`, and `ActionMerge` in `internal/tui/service.go`.
- Small value structs carry domain words directly: `StatusEntry{Code, Path}`, `tui.Action{Target, Kind}`, and `process.IO{Stdin, Stdout, Stderr, Dir}`.
...
- Service boundaries add action and target context with `%w`, such as source git and lazygit errors in `internal/cli/service.go`.
...
- `internal/tui/service.go` defines `ReviewService` as the TUI-facing contract: `Status`, `DiffOutput`, `ExecuteNonInteractive`, and `TerminalCommand`.
...
- Test doubles are hand-written fakes in `_test.go`, named by behavior rather than framework: `fakeRunner` in `internal/chezmoi/client_test.go`, `recordingRunner` in `internal/cli/service_test.go`, `fakeService` in `internal/cli/cli_test.go`, `fakeContentSource` in `internal/diff/diff_test.go`, and `fakeReviewService` in `internal/tui/sync_test.go`.
...
- `internal/app` owns process options and runtime version metadata.
...
- `renderDiff` in `internal/cli/diff_cmd.go` expands no-target invocations by calling `svc.Status(nil)` and diffing each dirty `entry.Path`; explicit targets bypass that status lookup.
